...
class TensorcadeWrapper:
    ...
    def __init__(
        self,
        model_path: str,
        config: Optional[TensorcadeConfig] = None,
        ...
        print(f"[TensorcadeWrapper] Config: nfft={self.config.nfft}, "
...
if __name__ == "__main__":
    ...
    fake_iq = (np.random.randn(chunk_samples) + 1j * np.random.randn(chunk_samples)).astype(np.complex64)
    ...
    if len(sys.argv) >= 3:
        ...
        try:
            ...
            print(f"✓ Real data inference works")
            print(f"  - Detections: {len(detections)}")
            for d in detections:
