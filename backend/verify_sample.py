...
logger = get_logger("verify_sample")


def verify_sample(
    signal_name: str, sample_id: str = None, training_data_dir: str = "training_data/signals"
):
    """
    Verify that training sample boxes land on bright signal regions.
    ...
    if not base_dir.exists():
        logger.error(f"ERROR: No samples found at {base_dir}")
        return False
    ...
    if sample_id:
        ...
        if not npz_files:
            logger.error(f"ERROR: No .npz files found in {base_dir}")
            return False
        samples = [f.stem for f in npz_files[:5]]

    logger.info(f"\n{'=' * 60}")
    logger.info(f"TRAINING SAMPLE VERIFICATION: {signal_name}")
    logger.info(f"{'=' * 60}")
    ...
    for sid in samples:
        ...
        logger.info(f"  Spectrogram range: {spec.min()}-{spec.max()}")
        logger.info(f"  Coordinate format: {coord_format}")
        ...
        for i, box in enumerate(boxes):
            ...
            if box_region.size == 0:
                logger.info(f"  Box {i}: EMPTY REGION (coords out of bounds)")
                sample_pass = False
                continue
            ...
            logger.info(f"    Size: {x_max - x_min}x{y_max - y_min}")
            logger.info(f"    Mean brightness: {mean_val:.1f}")
            logger.info(f"    Range: {min_val}-{max_val}")
            logger.info(f"    {status} (expect >120 for signal)")
    ...
    logger.info(f"Samples verified: {total}")
    logger.info(f"Passed: {passed}/{total}")

    if all_pass:
        logger.info("\n✅ ALL SAMPLES PASS - Boxes land on signal regions!")
        logger.info("   The coordinate fix is working correctly.")
    else:
        logger.error("\n❌ SOME SAMPLES FAIL - Boxes may be misaligned!")
        logger.info("   Check the verification images in training_data/verification/")
        logger.info("\n   Possible causes:")
        logger.info("   1. Old samples created before the coordinate fix")
        logger.info("   2. Flutter not sending real-unit coordinates")
        logger.info("   3. Python not receiving/parsing real units correctly")
...
def verify_all_signals(training_data_dir: str = "training_data/signals"):
    ...
    if not base_dir.exists():
        logger.error(f"ERROR: Training data directory not found: {base_dir}")
        return

    signals = [d.name for d in base_dir.iterdir() if d.is_dir()]

    if not signals:
        logger.info(f"No signals found in {base_dir}")
        return
