...
logger = get_logger("sample_manager")
...
class SampleManager:
    ...
    def _box_to_pixels(
        ...
        with open(debug_log, "a") as f:
            f.write(
                f"[_box_to_pixels] INPUT: box_freq={box['freq_start_mhz']:.2f}-{box['freq_end_mhz']:.2f} MHz\n"
            )
            f.write(
                f"[_box_to_pixels] PARAMS: center={center_freq_hz / 1e6:.2f} MHz, bw={bandwidth_hz / 1e6:.2f} MHz\n"
            )
            f.write(
                f"[_box_to_pixels] RANGE: freq_min={freq_min_hz / 1e6:.2f}, freq_max={freq_max_hz / 1e6:.2f} MHz\n"
            )
    ...
    def _generate_sample_id(
        self, signal_name: str, source_file: str, time_offset_sec: float
    ) -> str:
        """
    ...
    def save_sample(
        self, signal_name: str, iq_data_b64: str, boxes: list[dict], metadata: dict
    ) -> tuple[str, bool]:
        """
        ...
        center_freq_hz = metadata.get(
            "center_freq_hz", metadata.get("center_freq_mhz", 825.0) * 1e6
        )
        ...
        debug_log(
            f"NOTE: File header claimed bandwidth={file_claimed_bw / 1e6:.2f} MHz, using sample_rate={bandwidth_hz / 1e6:.2f} MHz for spectrogram"
        )
        ...
        if has_real_units and boxes:
            ...
            if rfcap_path and os.path.exists(rfcap_path):
                try:
                    ...
                    logger.info(f"[SampleManager] Extracted {len(iq_data)} samples centered on box")
                except Exception as e:
                    logger.error(f"[SampleManager] Failed to read rfcap, using provided IQ: {e}")
            ...
            debug_log(
                f"  POSITIVE FREQ ONLY: center={center_freq_hz / 1e6:.2f} MHz, half_bw={freq_range_hz / 1e6:.2f} MHz"
            )
            debug_log(f"  Displayed range: {freq_min_hz / 1e6:.2f} - {freq_max_hz / 1e6:.2f} MHz")
            debug_log(
                f"  Time window: {window_start:.4f}s - {window_start + window_duration:.4f}s ({window_duration * 1000:.1f}ms)"
            )
            debug_log(f"  Output size: {w}x{h}")

            for i, box in enumerate(boxes):
                debug_log(f"--- BOX {i} ---")
                debug_log("  FLUTTER SENT:")
                debug_log(f"    time: {box['time_start_sec']:.4f}s - {box['time_end_sec']:.4f}s")
                debug_log(
                    f"    freq: {box['freq_start_mhz']:.4f} MHz - {box['freq_end_mhz']:.4f} MHz"
                )
                ...
                debug_log("  FREQ CONVERSION:")
                debug_log(
                    f"    box freq Hz: {box_freq_low_hz / 1e6:.4f} - {box_freq_high_hz / 1e6:.4f} MHz"
                )
                ...
                debug_log(
                    f"    normalized (0=low freq, 1=high freq): {freq_frac_low:.4f} - {freq_frac_high:.4f}"
                )
                ...
                debug_log(
                    f"  FINAL PIXEL BOX: x={pixel_box['x_min']}-{pixel_box['x_max']}, y={pixel_box['y_min']}-{pixel_box['y_max']}"
                )
                ...
                logger.info(
                    f"[SampleManager] Box real: t={box['time_start_sec']:.3f}-{box['time_end_sec']:.3f}s, "
                    f"f={box['freq_start_mhz']:.2f}-{box['freq_end_mhz']:.2f}MHz"
                )
                logger.info(
                    f"[SampleManager] Box pixel: x={pixel_box['x_min']}-{pixel_box['x_max']}, "
                    f"y={pixel_box['y_min']}-{pixel_box['y_max']}"
                )
        else:
            # OLD FORMAT: Use provided IQ and normalized coords (backwards compat)
            logger.warning(
                "[SampleManager] WARNING: Using old normalized coord format - results may be incorrect!"
            )
    ...
    def _read_iq_from_rfcap(
        self, rfcap_path: str, start_sample: int, num_samples: int
    ) -> np.ndarray:
        """
    ...
    def _update_manifest(self, signal_name: str, sample_id: str, metadata: dict):
        ...
        if source_file:
            source_entry = next(
                (s for s in manifest["sources"] if s["capture_file"] == source_file), None
            )
            if source_entry:
    ...
    def get_sample_info(self, signal_name: str, sample_id: str) -> dict | None:
    ...
    def get_manifest(self, signal_name: str) -> dict | None:
