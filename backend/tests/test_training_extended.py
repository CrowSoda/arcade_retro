...
class TestRandomHorizontalFlip:
    ...
    def test_flip_with_p1(self):
        """Flip always applied when p=1."""
        from training.dataset import RandomHorizontalFlip
        ...
        target = {
            "boxes": torch.tensor([[0, 10, 10, 20]], dtype=torch.float32),
        ...
        assert flipped_image[:, :, -1].sum() > 0  # Now right column has 1s

        # Check boxes were flipped (x coords)
        # Original: [0, 10, 10, 20] -> width=64, so [54, 10, 64, 20]
        expected_box = torch.tensor([[54, 10, 64, 20]], dtype=torch.float32)
        torch.testing.assert_close(flipped_target["boxes"], expected_box)

    def test_no_flip_with_p0(self):
        """No flip when p=0."""
        from training.dataset import RandomHorizontalFlip
        ...
        # Should be unchanged
        torch.testing.assert_close(flipped_target["boxes"], target["boxes"])
...
class TestRandomVerticalFlip:
    ...
    def test_flip_with_p1(self):
        """Flip always applied when p=1."""
        from training.dataset import RandomVerticalFlip

        flip = RandomVerticalFlip(p=1.0)

        # Create test image
        image = torch.zeros(3, 64, 64)
        image[:, 0, :] = 1.0  # Top row is 1

        target = {
            "boxes": torch.tensor([[10, 0, 20, 10]], dtype=torch.float32),
            "labels": torch.tensor([1], dtype=torch.int64),
        }
        ...
        # Check image was flipped
        assert flipped_image[:, -1, :].sum() > 0  # Now bottom row has 1s

        # Check boxes were flipped (y coords)
        # Original: [10, 0, 20, 10] -> height=64, so [10, 54, 20, 64]
        expected_box = torch.tensor([[10, 54, 20, 64]], dtype=torch.float32)
        torch.testing.assert_close(flipped_target["boxes"], expected_box)
...
class TestCompose:
    ...
    def test_compose_multiple_transforms(self):
        ...
        from training.dataset import Compose

        call_order = []

        def transform_a(image, target):
            call_order.append("a")
            return image, target

        def transform_b(image, target):
            call_order.append("b")
            return image, target
