...
class ExtractionParams:
    ...
    target_rate: Optional[float] = None  # Hz, None = auto (2.5x BW)
...
class SubbandExtractor:
    ...
    def __init__(self, params: ExtractionParams):
        ...
        if self.target_rate < min_rate:
            print(f"[SubbandExtractor] WARNING: Target rate {self.target_rate/1e6:.2f} MHz "
    ...
    def extract(self, iq_data: np.ndarray, 
                progress_callback: Optional[Callable[[float], None]] = None) -> ExtractionResult:
        """
        ...
        if self.up != 1 or self.down != 1:
            iq_decimated = resample_poly(iq_data, self.up, self.down, 
...
def _read_rfcap_header(filepath: str) -> dict:
    ...
    if magic[:4] == G20_MAGIC:
        print(f"[SubbandExtractor] Detected G20 format: {filepath}")
        return _read_g20_header(filepath)
    elif magic == RFCAP_MAGIC:
        print(f"[SubbandExtractor] Detected legacy RFCAP format: {filepath}")
        return _read_legacy_rfcap_header(filepath)
    else:
        raise ValueError(f"Unknown header format. Magic: {magic!r}. Expected G20 or RFCAP.")


def _read_iq_data(filepath: str, offset_samples: int = 0, num_samples: Optional[int] = None) -> np.ndarray:
...
def _write_g20_header(filepath: str, metadata: dict):
    ...
    print(f"[SubbandExtractor] Wrote G20 header: sr={metadata['sample_rate']/1e6:.2f}MHz, "
...
def extract_subband_from_file(
    ...
    report(0.10, "reading")
    print(f"[SubbandExtractor] Read {len(iq_data)/1e6:.2f}M samples")
    ...
    print(f"[SubbandExtractor] Phase 1 done: frequency shifted")
    ...
    print(f"[SubbandExtractor] Resampling: {sample_rate/1e6:.2f}MHz → {actual_target_rate/1e6:.2f}MHz ({up}:{down})")
    ...
    if up != 1 or down != 1:
        ...
        print(f"[SubbandExtractor] Filter: {len(filter_taps)} taps")
        ...
        for i in range(num_chunks):
            ...
            print(f"[SubbandExtractor] Resampled chunk {i+1}/{num_chunks} ({chunk_progress*100:.0f}%)")
    ...
    print(f"[SubbandExtractor] Phase 2 done: {len(iq_resampled)/1e6:.2f}M output samples")
    ...
    print(f"[SubbandExtractor] COMPLETE in {elapsed:.1f}s: "
