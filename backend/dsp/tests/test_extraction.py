...
class TestFilterDesign:
    ...
    def test_passband_ripple(self):
        ...
        if np.any(passband_mask):
            ...
            assert (
                passband_ripple < 0.5
            ), f"Passband ripple {passband_ripple:.2f} dB (expected < 0.5 dB)"
    ...
    def test_minimum_taps(self):
        ...
        assert (
            len(extractor.filter_taps) >= 63
        ), f"Filter has only {len(extractor.filter_taps)} taps"
...
class TestDecimation:
    ...
    def test_decimation_ratio_4x(self):
        ...
        test_data = (np.random.randn(input_samples) + 1j * np.random.randn(input_samples)).astype(
            np.complex64
        )
