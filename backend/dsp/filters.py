...
def analyze_filter(
    ...
    passband_ripple = mag_db[passband_mask].max() - mag_db[passband_mask].min() if np.any(passband_mask) else 0
...
def design_cic_compensation_filter(
    ...
    eps = 1e-10
    h_cic = np.abs(np.sin(np.pi * f * decimation + eps) / 
                   (decimation * np.sin(np.pi * f + eps))) ** cic_order
