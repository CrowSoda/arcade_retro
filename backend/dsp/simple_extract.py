...
def write_rfcap_header(f, center_freq_hz: float, sample_rate: float, 
                       bandwidth_hz: float, num_samples: int, signal_name: str = 'SBT'):
...
def extract_subband(
    ...
    print(f"[extract_subband] CHUNKED processing: {chunk_seconds}s chunks", flush=True)
    print(f"[extract_subband] Input: {input_path}", flush=True)
    print(f"[extract_subband] Original: center={original_center_hz/1e6:.3f} MHz, rate={original_sample_rate/1e6:.3f} Msps", flush=True)
    print(f"[extract_subband] Target: center={new_center_hz/1e6:.3f} MHz, bw={new_bandwidth_hz/1e6:.3f} MHz", flush=True)
    _progress(0.02)
    ...
    print(f"[extract_subband] Filter: cutoff={cutoff_hz/1e6:.3f} MHz, normalized={cutoff_normalized:.4f}", flush=True)
    print(f"[extract_subband] Decimation: {decimation}x (output rate: {actual_output_rate/1e6:.2f} Msps)", flush=True)
    ...
    print(f"[extract_subband] Total: {total_input_samples} samples in {total_chunks} chunks of {chunk_samples} samples", flush=True)
    _progress(0.08)
    ...
    with open(input_path, 'rb') as f_in, open(output_path, 'wb') as f_out:
        ...
        for chunk_idx in range(total_chunks):
            ...
            if chunk_idx % 10 == 0 or chunk_idx == total_chunks - 1:
                print(f"[extract_subband] Chunk {chunk_idx+1}/{total_chunks}: {total_output_samples} output samples", flush=True)
    
    # Update header with actual sample count
    update_sample_count(output_path, total_output_samples)
    
    print(f"[extract_subband] Complete! Output: {total_output_samples} samples", flush=True)
    print(f"[extract_subband] Output header: sr={actual_output_rate/1e6:.2f}MHz, bw={actual_output_rate/1e6:.2f}MHz", flush=True)
    print(f"[extract_subband] Saved to: {output_path}", flush=True)
    _progress(1.0)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 6:
        ...
        print("Usage: python simple_extract.py <input> <output> <orig_center_hz> <orig_rate> <new_center_hz> [new_bw_hz]")
