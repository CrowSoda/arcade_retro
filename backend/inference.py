...
class InferenceEngine:
    ...
    def infer(self, images: torch.Tensor, score_threshold: float = 0.9) -> list[dict[str, Any]]:
        """
    ...
    def _parse_detections(
        self, outputs: list[np.ndarray], batch_size: int, threshold: float
    ) -> list[dict]:
        ...
        if len(outputs) >= 3:
            ...
            results.append({"boxes": boxes[mask], "scores": scores[mask], "labels": labels[mask]})
...
class MultiModelEngine:
    ...
    def __init__(
        self, model_configs: list[tuple[str, int]], device: str = "cuda", precision: str = "fp16"
    ):
        self.device = torch.device(device if torch.cuda.is_available() else "cpu")
        self.precision = precision
    ...
    def infer_parallel(
        self, images: torch.Tensor, score_threshold: float = 0.9
        ...
        if self.device.type == "cuda":
            for i, (engine, stream) in enumerate(zip(self.engines, self.streams, strict=False)):
    ...
    def infer_merged(
        self, images: torch.Tensor, score_threshold: float = 0.9, nms_threshold: float = 0.5
    ) -> list[dict[str, Any]]:
...
class SpectrogramPipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"SpectrogramPipeline: nfft={nfft}, noverlap={noverlap}, "
    ...
    def process(self, iq_data: np.ndarray) -> torch.Tensor:
        ...
        resized = F.interpolate(
            sxx_norm, size=(self.out_size, self.out_size), mode="bilinear", align_corners=False
        )
...
if __name__ == "__main__":
    ...
    if args.benchmark:
        ...
        logger.info(f"Backend: {engine.backend}")
    ...
    if args.benchmark_multi:
        ...
        logger.info(f"  Sequential: {results['sequential_ms']:.2f} ms")
        logger.info(f"  Parallel:   {results['parallel_ms']:.2f} ms")
