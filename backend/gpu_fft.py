...
class GPUSpectrogramProcessor:
    ...
    def __init__(self, fft_size: int = DEFAULT_FFT_SIZE, device: str = "cuda"):
        ...
        logger.info(f"[GPUSpectrogramProcessor] Initialized: FFT={fft_size}, device={self.device}")
    ...
    def _warmup(self):
        ...
        logger.info(
            f"[GPUSpectrogramProcessor] Warming up cuFFT kernels for FFT size {self.fft_size}..."
        )
        ...
        dummy_batch_size = 25
        dummy = torch.randn(
            dummy_batch_size, self.fft_size, dtype=torch.complex64, device=self.device
        )
        ...
        self.is_warming_up = False
        logger.info(f"[GPUSpectrogramProcessor] Warmup complete ({warmup_ms:.1f}ms)")
    ...
    def update_fft_size(self, new_size: int) -> dict:
        ...
        old_size = self.fft_size
        logger.info(f"[GPUSpectrogramProcessor] Updating FFT size: {old_size} -> {new_size}")
    ...
    def process(self, iq_data: np.ndarray, decimate_to: int = 0) -> np.ndarray:
        ...
        segments = as_strided(
            iq_data, shape=(num_ffts, self.fft_size), strides=(self.hop_size * itemsize, itemsize)
        )
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("GPU FFT Processor Test")
    logger.info("=" * 60)
    ...
    # Create processor
    proc = GPUSpectrogramProcessor(fft_size=32768)
    logger.info(f"Processor info: {proc.get_info()}")
    ...
    for fft_size in [8192, 16384, 32768, 65536]:
        ...
        logger.info(
            f"  FFT {fft_size:5d}: {num_ffts:2d} FFTs, avg={avg_time:.1f}ms, min={min_time:.1f}ms, shape={result.shape}"
        )
