...
def extract_backbone_and_head(
    ...
    print(f"Backbone keys: {len(backbone_state)}")
    print(f"Head keys: {len(head_state)}")
    ...
    print(f"Saved backbone to: {backbone_output_path}")
    
    # Save head (only RPN + ROI heads)
    torch.save(head_state, head_output_path)
    print(f"Saved head to: {head_output_path}")
...
def create_metadata_files(
    ...
    print(f"Created backbone metadata: {backbone_meta_path}")
    ...
    print(f"Created head metadata: {head_meta_path}")
    ...
    if sys.platform == "win32":
        if v1_path.exists() and not active_path.exists():
            ...
            print(f"Copied v1.pth to active.pth")
    else:
        if v1_path.exists() and not active_path.exists():
            active_path.symlink_to("v1.pth")
            print(f"Created symlink active.pth -> v1.pth")
...
def validate_extraction(
    ...
    if len(original_boxes) != len(hydra_boxes):
        print(f"WARNING: Different number of detections: {len(original_boxes)} vs {len(hydra_boxes)}")
    ...
    if len(original_boxes) > 0 and len(hydra_boxes) > 0:
        ...
        print(f"Top box difference: {box_diff:.6f}")
        print(f"Top score difference: {score_diff:.6f}")
        ...
        print("No detections to compare (may be valid for random input)")
        return True
...
def run_migration(models_dir: str, source_model: str = None):
    ...
    if source_model is None:
        ...
        if source_model is None:
            print("ERROR: No source model found. Specify with --input")
            print(f"Looked in: {[str(c) for c in candidates]}")
            return False
    
    print(f"\n{'='*60}")
    print("HYDRA MIGRATION: Extract backbone + head from full model")
    print(f"{'='*60}")
    ...
    if backbone_path.exists() and head_path.exists():
        print("Migration already complete. Files exist:")
    ...
    print(f"\nExtraction complete:")
    print(f"  Backbone: {info['backbone_size_mb']:.1f} MB ({info['backbone_keys']} keys)")
    print(f"  Head: {info['head_size_mb']:.1f} MB ({info['head_keys']} keys)")
    ...
    print(f"{'='*60}")
    print("\nNew structure:")
    print(f"  models/backbone/active.pth  ({info['backbone_size_mb']:.1f} MB)")
    print(f"  models/heads/creamy_chicken/active.pth  ({info['head_size_mb']:.1f} MB)")
    print(f"  models/heads/creamy_chicken/metadata.json")
...
def main():
    ...
    parser.add_argument("--validate-only", action="store_true", help="Only validate existing extraction")
