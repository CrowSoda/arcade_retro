...
def write_rfcap_header(f, sample_rate: float, center_freq: float, bandwidth: float,
                       num_samples: int, start_time: float, signal_name: str,
...
def read_rfcap_header(filepath: str) -> dict:
    ...
    # Unpack
    (magic, version, sample_rate, center_freq, bandwidth, num_samples,
     start_time, name_bytes, latitude, longitude) = struct.unpack(
        '<4sI d d d Q d 32s d d', raw[:96]
    )
...
def resample_iq(iq_complex: np.ndarray, source_rate: float, target_rate: float) -> np.ndarray:
    ...
    print(f"    Resampling: {source_rate/1e6:.1f} MHz -> {target_rate/1e6:.1f} MHz (up={up}, down={down})")
...
def filter_and_extract_subband(iq_complex: np.ndarray, source_rate: float,
                                center_offset_hz: float, target_bw_hz: float) -> tuple:
    """
...
def main():
    ...
    for i, sig in enumerate(DEMO_SIGNALS, 1):
        ...
        target_sample_rate = target_bw_hz  # For complex, sample_rate = bandwidth
        ...
        print(f"    Reading {bytes_to_read / 1e6:.1f} MB ({samples_to_read:,} samples) from offset {offset_bytes / 1e6:.1f} MB...")
        ...
        print(f"    Loaded {actual_samples_read:,} complex samples ({actual_samples_read / SOURCE_SAMPLE_RATE:.2f}s at source rate)")
        
        # Sub-band extraction and resampling
        if target_bw_hz < SOURCE_SAMPLE_RATE:
            print(f"    Sub-band tuning: BW {SOURCE_SAMPLE_RATE/1e6:.1f} -> {target_bw_hz/1e6:.1f} MHz, offset {center_offset_hz/1e6:.1f} MHz")
        ...
        print(f"    Output: {actual_num_samples:,} samples at {output_sample_rate/1e6:.1f} MHz = {actual_duration:.2f}s")
        ...
        print(f"    Verified: {header['signal_name']}, sr={header['sample_rate']/1e6:.1f}MHz, "
